"""fix wrong rtsp_streaming__ prefix → rtsp_detection__
...
def upgrade() -> None:
    """잘못 붙은 'rtsp_streaming__' prefix 를 'rtsp_detection__' 로 교체.

    선두 prefix 만 한 번 교체(idempotent — rtsp_streaming__ 로 시작하는 행만 매칭).
    """
    op.execute(
        "UPDATE ip_cams "
        "SET stream_key = 'rtsp_detection__' || substr(stream_key, length('rtsp_streaming__') + 1) "
...
def downgrade() -> None:
    ...
    op.execute(
        "UPDATE ip_cams "
        "SET stream_key = 'rtsp_streaming__' || substr(stream_key, length('rtsp_detection__') + 1) "
        "WHERE stream_key LIKE 'rtsp_detection__%'"
    )
