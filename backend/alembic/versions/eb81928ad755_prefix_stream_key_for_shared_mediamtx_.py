...
def upgrade() -> None:
    ...
    """
    op.execute(
        "UPDATE ip_cams SET stream_key = 'rtsp_detection__' || stream_key "
        "WHERE stream_key NOT LIKE 'rtsp_detection__%'"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "UPDATE ip_cams SET stream_key = substr(stream_key, length('rtsp_detection__') + 1) "
