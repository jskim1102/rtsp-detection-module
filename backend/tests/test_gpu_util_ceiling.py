...
def test_worker_pool_duty_combines_model_lane_ewmas():
    ...
    worker._observe_lane_duty(
        InferenceResult("a", 1.0, infer_ms=50.0, idle_ms=50.0, model="yolo26n.pt")
    )
    worker._observe_lane_duty(
        InferenceResult("b", 1.0, infer_ms=50.0, idle_ms=50.0, model="yolo26x.pt")
    )
...
def test_worker_pool_duty_ignores_warmup_and_retires_stale_lane():
    worker = InferenceWorker()
    worker._observe_lane_duty(
        InferenceResult("a", 1.0, infer_ms=20.0, idle_ms=20.0, model="yolo26n.pt")
    )
    worker._observe_lane_duty(
        InferenceResult("b", 1.0, infer_ms=0.0, idle_ms=0.0, model="yolo26x.pt")
    )

    assert worker.get_pool_duty() == pytest.approx(0.5)

    worker.configure_models({"yolo26x.pt"})
