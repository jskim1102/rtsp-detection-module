...
def test_is_alive_requires_every_desired_model_lane():
    worker = _pool(
        {
            "yolo26n.pt": _FakeLane(alive=True),
            "yolo26x.pt": _FakeLane(alive=False),
        }
    )

    assert worker.is_alive() is False
    worker._lanes["yolo26x.pt"].alive = True
    assert worker.is_alive() is True


def test_start_after_stop_restarts_desired_lanes():
    lane = _FakeLane(alive=True)
    worker = _pool({"yolo26n.pt": lane})
...
def test_configure_models_stops_unused_lane_releasing_vram():
    ...
    worker = _pool({"yolo26n.pt": nano, "yolo26x.pt": extra})

    worker.configure_models({"yolo26x.pt"})

    assert set(worker._lanes) == {"yolo26x.pt"}
    assert nano.stops == 1
