...
def test_multimodel_merge_preserves_request_frame_coordinate_contract():
    ...
    first = InferenceResult(
        "cam",
        123.0,
        detections=[Detection(0, "person", 0.9, (1, 2, 30, 40), "yolo26n.pt")],
        frame_w=640,
        frame_h=360,
        infer_ms=4.0,
        idle_ms=2.0,
    ...
    second = InferenceResult(
        "cam",
        123.0,
        detections=[Detection(2, "car", 0.8, (10, 20, 300, 200), "yolo26x.pt")],
        frame_w=640,
        frame_h=360,
        infer_ms=12.0,
        idle_ms=1.0,
    ...
    assert [d.model for d in merged.detections] == ["yolo26n.pt", "yolo26x.pt"]
...
def test_inflight_aggregates_are_source_bounded_and_do_not_retain_frames():
    ...
    lane = worker._make_lane("yolo26n.pt")
    lane.in_q = SinkQueue()
    worker._lanes["yolo26n.pt"] = lane
...
def test_pool_aggregates_two_model_lane_results_once():
    worker = InferenceWorker()
    for model in ("yolo26n.pt", "yolo26x.pt"):
    ...
    worker.submit(
        FrameRequest(
            ...
            frame,
            42.0,
            model_names=["yolo26n.pt", "yolo26x.pt"],
            imgsz=640,
        )
    )
    worker.flush_pending()
