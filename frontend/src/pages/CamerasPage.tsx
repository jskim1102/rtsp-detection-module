...
export default function CamerasPage() {
  ...
  // 카메라별 — 모델별 통합 설정(conf override + class filter + class colors). frontend overlay 적용.
  ...
  // 모델 lane 전체의 실제 busy duty를 1초마다 표시. target은 최초 1회만 서버값으로
  ...
  // 요청 수명주기는 순수 updater에 위임해 DOM 없이 debounce/abort/rollback을 회귀 테스트한다.
  useEffect(() => {
  ...
  // 모달 conf 슬라이더는 BboxOverlay 표시필터(per-model)만이 아니라 워커 추론 conf 도 낮춰야 한다:
  // 워커가 서버에서 conf 미만을 이미 버리면 클라 필터를 내려도 보여줄 검출이 없기 때문(글로벌 0.5 고정 버그).
  // 워커 conf = 선택 모델 conf 의 최솟값(모든 모델이 원하는 최저 임계값까지 emit) → 표시는 per-model 필터가 정제.
  ...
  return (
    <main className="app">
      <header className="page-head">
        <div>
          <h1>RTSP Detection</h1>
          <p className="subtitle">카메라 관리 · YOLO 객체탐지</p>
        </div>
        <button
          className="primary"
          disabled={atCap}
      ...
      <section className="gpu-util-control" aria-labelledby="gpu-util-label">
        <div className="gpu-util-copy">
          ...
          <p>카메라 수와 모델 부하에 맞춰 det FPS를 자동 배분합니다.</p>
        </div>
      ...
      <section className="live-section">
        ...
        <CameraGrid
          ...
          settingsByCam={modelSettingsByCam}
        />
      </section>
